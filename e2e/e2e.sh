...
if [ -z "$LAF_DEPLOYMENT_TIMEOUT" ]; then
  LAF_DEPLOYMENT_TIMEOUT=60
fi

if [ -z "$LAF_APP_CREATION_TIMEOUT" ]; then
  LAF_APP_CREATION_TIMEOUT=20
fi

if [ -z "$LAF_SERVER_RESTART_TIMEOUT" ]; then
  LAF_SERVER_RESTART_TIMEOUT=20
fi
...
# sudo sh install-on-linux.sh $DOMAIN
...
response=$(curl -X 'GET' 'http://api.10.59.179.7.nip.io/v1/regions' -H 'accept: */*')
...
response=$(curl -X 'GET' 'http://api.10.59.179.7.nip.io/v1/runtimes' -H 'accept: */*')
...
curl "${API_ENDPOINT}/v1/subscriptions" \
...
for i in $(seq 1 $MAX_RETRIES); do
  ...
  response=$(curl -s -X 'GET' "http://api.10.59.179.7.nip.io/v1/applications" \
    -H 'accept: */*' \
    -H "Authorization: Bearer ${LAF_TOKEN}")
...
set +e
kubectl wait --for=condition=Ready pod --all -n ${APP_ID} --timeout=${LAF_APP_CREATION_TIMEOUT}s
if [ $? -ne 0 ]; then
...
echo "Restart laf-server(because we should restart laf-server after creating the first app)"

kubectl delete pods -l app.kubernetes.io/name=laf-server -n laf-system

echo "Wait for Laf server to be ready"

kubectl wait --for=condition=Ready pod --all -n laf-system --timeout=${LAF_SERVER_RESTART_TIMEOUT}s
if [ $? -ne 0 ]; then
  echo "laf-server pod in laf-system namespace are not running\n"
  echo "try to describe laf-server pod in laf-system namespace whose status is not Running\n"
  kubectl get pods -n laf-system --field-selector=status.phase!=Running -o=name | xargs kubectl describe -n laf-system
  echo "try to log laf-server pod in laf-system namespace whose status is not Running\n"
  kubectl get pods -n laf-system --field-selector=status.phase!=Running -o=name | xargs kubectl logs -n laf-system
  exit 1
fi
...
curl "${API_ENDPOINT}/v1/apps/${APP_ID}/functions" \
  -H 'Accept: application/json, text/plain, */*' \
  ...
  -H 'Content-Type: application/json' \
  -H 'DNT: 1' \
  --data-raw $'{"name":"f1","description":"laf e2e function","websocket":false,"methods":["GET","POST"],"code":"import cloud from \'@lafjs/cloud\'\\n\\nexport async function main(ctx: FunctionContext) {\\n  console.log(\'Hello World\')\\n  return { data: \'hi, laf\' }\\n}","tags":["e2e"]}' \
  --compressed \
  --insecure
...
curl "${API_ENDPOINT}/v1/apps/${APP_ID}/functions" \
  ...
  -H 'Content-Type: application/json' \
  -H 'DNT: 1' \
  --data-raw $'{"name":"f2","description":"laf e2e function","websocket":false,"methods":["GET","POST"],"code":"import cloud from \'@lafjs/cloud\'\\n\\nexport async function main(ctx: FunctionContext) {\\n  console.log(\'Hello World\')\\n  return { data: \'hi, laf\' }\\n}","tags":["e2e"]}' \
  --compressed \
  --insecure
...
curl "${API_ENDPOINT}/v1/apps/${APP_ID}/functions/f2" \
  -X 'PATCH' \
  -H 'Accept: application/json, text/plain, */*' \
  -H 'Accept-Language: zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7' \
  -H "Authorization: Bearer ${LAF_TOKEN}" \
